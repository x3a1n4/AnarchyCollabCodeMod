    ...
    public class DreamSpring : Entity {
        ...
        public DreamSpring(EntityData data, Vector2 offset) {
            ...
            if(!Diagonal) {
                ...
                if(Rotated) Collider = new Hitbox(6f, 16f, Flipped ? -6f : 0f, -8f);
                else Collider = new Hitbox(16f, 6f, -8f, Flipped ? 0 : -6f);
            } else {
                sprite.Origin = new Vector2(0f, 0f);
                sprite.Scale.Y = Flipped ? 1 : -1;
                sprite.Rotation = Rotated ? 0 : ((float) Math.PI / 2 * sprite.Scale.Y);

                if(Rotated) Collider = new Hitbox(16f, 16f, -16f, Flipped ? -16f : 0f);
                else Collider = new Hitbox(16f, 16f, 0f, Flipped ? -16f : 0f);
            }
    	    Depth = -11001;
        ...
        private void OnPlayerCollision(Player player) {
            if(player.StateMachine.State != Player.StDreamDash) return;
