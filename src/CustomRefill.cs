...
namespace Celeste.Mod.AnarchyCollab2022 {
  public abstract class CustomRefill : Refill {
    ...
    private static Color ShiftColor(Color color, Matrix hueShift, float intensityShift) {
      ...
      if (color.IsApproximately(Color.White)) {
        return new Color(new Vector4(shiftedRGB, color.A));
      } else {
        return new Color(new Vector4(shiftedRGB * intensityShift, color.A));
      }
    }

    private static ParticleType ShiftColor(ParticleType type, Matrix hueShift, float intensityShift) {
      return new ParticleType(type) {
